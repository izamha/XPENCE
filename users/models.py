...
class CustomUser(AbstractUser):
    username = None
    phone_regex = RegexValidator(regex=r'^\+?(250)\d{9,14}',
                                 message=_("Phone Number should be of format: +250xxxxxxxx"))
    phone = models.CharField(validators=[phone_regex], max_length=15, unique=True, null=True,
                             error_messages={'required': _('Please enter your phone number'),
                                             'unique': _('There is someone else using this number.')})
...
class Profile(models.Model):
    ...
    image = models.ImageField(default='default.jpg', upload_to='profiles', null=True)
